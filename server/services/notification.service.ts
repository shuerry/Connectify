...
<body style="margin:0;padding:0;background:#f6f8fb;">
          ...
          <tr>
            <td style="padding:12px 24px 20px 24px;border-top:1px solid #e5e7eb;">
              <p style="margin:0;font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Cantarell,'Helvetica Neue',sans-serif;font-size:12px;line-height:18px;color:#6b7280;">
                ${footerNote ? this.escape(footerNote) : `Sent by ${this.escape(this.siteUrl)} • Please don’t reply to this automated email.`}
              </p>
            </td>
          </tr>
        </table>
  ...
  async sendChatNotification(payload: ChatNotificationPayload) {
    ...
    const link = chatId ? `${this.siteUrl}/chat/${chatId}` : this.siteUrl;

    const intro = toName
      ? `Hi ${this.escape(toName)},`
      : `Hello,`;

    const body = `
      <p style="margin:0 0 8px 0;"><strong>${this.escape(fromName || 'Someone')}</strong> wrote:</p>
      ...
      ${groupName ? `<p style="margin:12px 0 0 0;">Conversation: <strong>${this.escape(groupName)}</strong></p>` : ''}
    `;
  ...
  async sendAnswerNotification(payload: AnswerNotificationPayload) {
    ...
    const link = `${this.siteUrl}/question/${questionId}`;

    const intro = `Hi,`;
    const body = `
      <p style="margin:0 0 8px 0;">
        <strong>${this.escape(authorName || 'Someone')}</strong> posted a new answer${questionTitle ? ` on <em>${this.escape(questionTitle)}</em>` : ''}:
      </p>
      <blockquote style="margin:0;padding:8px 12px;border-left:3px solid #e5e7eb;background:#f9fafb;border-radius:6px;">
        ${this.escape(answerPreview || '')}
      </blockquote>
  ...
  async sendEmailVerification(payload: EmailVerificationPayload) {
    ...
    const link = verifyUrl || `${this.siteUrl}/verify-email?token=${encodeURIComponent(token || '')}`;
    const niceExpiry = expiresAt ? new Date(expiresAt).toLocaleString() : undefined;

    const subject = 'Verify your email address';
    const intro = username ? `Hi ${this.escape(username)},` : 'Hi,';
    const body = `
      <p style="margin:0 0 8px 0;">Please confirm that this email belongs to you by clicking the button below.</p>
      ${niceExpiry ? `<p style="margin:8px 0;">This link expires on <strong>${this.escape(niceExpiry)}</strong>.</p>` : ''}
    ...
    const html = this.layout({
      ...
      ctaHref: link,
      footerNote: `Sent by ${this.siteUrl} • Need help? Reply to ${this.fromEmail}`
    });
